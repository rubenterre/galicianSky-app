<script>
	...
	$effect(async () => {
		// Coordenadas aproximadas de Santiago de Compostela
		const lat = 42.8782;
		const lon = -8.5448;
		const [weather, forecast] = await Promise.all([
			obtenerDatos(lat, lon),
		]);
		dataWeather = weather;
		dataLocation = {
			lat: lat,
...
{#if dataWeather}
		...
		<main class="weather-app__main">
			<section class="weather-card">
				<div class="row">
					<div class="col s12">
						<header class="weather-card__header">
							<p class="weather-card__city">{dataWeather.name}</p>
				...
				<div class="row">
					<div class="col s12">
						<div class="weather-card__current">
							...
							<p class="weather-card__temperature">{Math.round(dataWeather.main.temp)}°</p>
							<p class="weather-card__description">{dataWeather.weather[0].description}</p>
				...
				<figure class="weather-card__landmark">
					<img
						src="./assets/cities/Santiago de Compostela.png"
						alt="Catedral de Santiago de Compostela"
						class="weather-card__landmark-image"
					/>
				</figure>

				<DetailsToday feelsLike = {dataWeather.main.feels_like} humidity = {dataWeather.main.humidity} wind = {dataWeather.wind.speed}></DetailsToday>
			</section>
		</main>
	</div>
	<section class="weather-card__forecast">
		...
		<ForecastHourly dataLocation = {dataLocation}></ForecastHourly>
	</section>
	<section class="weather-card__forecast">
		<header class="weather-card__forecast-header">
		...
		<ForecastDaily dataLocation = {dataLocation}></ForecastDaily>
	</section>
	...
	<section class="weather-card__details">
		...
		<div class="weather-card__details--container">
			<OthersDetails dataWeather = {dataWeather} dataLocation = {dataLocation}></OthersDetails>
		</div>
	</section>
...
<style>
	.weather-app {
		background:
			linear-gradient(180deg, rgba(0, 0, 0, 0.2) 0%, rgba(51, 51, 51, 0.95) 100%),
	...
	.weather-card__landmark {
		...
		justify-content: center;
		align-items: center;
		background: radial-gradient(rgba(255, 255, 255, 0.133) 0%, rgba(0, 0, 0, 0) 70%);
		padding: 1rem;
